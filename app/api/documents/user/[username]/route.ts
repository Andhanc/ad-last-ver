...
import { getUserFinalDocuments, getReportPdfPath } from "@/lib/local-storage"
...
// GET - Получение финальных версий документов пользователя
export async function GET(
  ...
  try {
    ...
    const documents = getUserFinalDocuments(username)
    const baseUrl = getBaseUrl(request)

    const documentsSummary = documents.map((doc) => {
      const hasReport = !!getReportPdfPath(doc.id)
