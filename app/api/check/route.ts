...
export async function POST(request: NextRequest) {
  try {
    ...
    // Получаем все документы из локальной БД (включая свои) для проверки.
    // Фильтр по учебному заведению (адресная проверка) выполняется в getAllDocumentsFromDb.
    let documents = getAllDocumentsFromDb(undefined, institution)

    // Дополнительный фильтр по типу работы:
    // - если проверяется курсовая или дипломная, сравниваем с обеими базами (coursework + diploma)
    // - для остальных типов — только с аналогичным типом (если указан)
    if (category === "coursework" || category === "diploma") {
      documents = documents.filter(
        (doc) => doc.category === "coursework" || doc.category === "diploma",
      )
